...
def search():
    try:
        ...
        query_text = data.get('interest', 'latest jobs')
        # Heavy search for more results
        query = f"{query_text} official portal 2025 2026 (site:.gov.in OR site:.nic.in OR site:org.in) -site:facebook.com"
        headers = {'X-API-KEY': SERPER_API_KEY, 'Content-Type': 'application/json'}
        # num 40 for lots of results
...
def chat():
    try:
        ...
        prompt = f"You are Rozgar AI by R YADAV PRODUCTION. User: {data.get('name')}. Give 6-hour study plans and job info: {data.get('message')}"
    ...
    except: return jsonify({"reply": "Bhai network slow hai, ek baar phir se pucho!"})
